...
describe("GovernanceBadge NFT", function () {
  ...
  it("check super admin NFT", async () => {
    let bal = await governanceBadgeNFT.callStatic.balanceOf(
      superAdmin.address,
      1
    );
    console.log({ bal });
  ...
  it("Request validation", async () => {
    ...
    let tx = await governanceBadgeNFT
      .connect(validatorAddress)
      .requestValidatorRole({ value: stakingAmount });
    await tx.wait();
    let validationRequestStake =
      await governanceBadgeNFT.callStatic.validationRequests(
        validatorAddress.address
      );
    console.log({ validationRequestStake, address: validatorAddress.address });
    expect(
      validationRequestStake.toString(),
      "Expected stake to be correct"
    ).to.eq(stakingAmount.toString());
  });
  ...
  it("vote validator", async () => {
    ...
    await tx.wait();
    let tx2 = await governanceBadgeNFT
      .connect(secondAdmin)
      .voteValidator(validatorAddress.address);
    await tx2.wait();
    let bal = await governanceBadgeNFT.callStatic.balanceOf(
      validatorAddress.address,
      1
  ...
  it("change stake amount", async () => {
    ...
    let tx = await governanceBadgeNFT.setStakingAmount(
      newStakeAmount.toString()
    );
    await tx.wait();
    let stakingAmount = await governanceBadgeNFT.callStatic.stakingAmount();
    expect(stakingAmount.toString(), "Stake amount not as expected").to.eq(
      newStakeAmount.toString()
    );
  });
  ...
  it("change minimum votes", async () => {
    ...
    expect(minVotes.toString(), "min votes not as expected").to.eq(
      newMinVotes.toString()
    );
  });
});
